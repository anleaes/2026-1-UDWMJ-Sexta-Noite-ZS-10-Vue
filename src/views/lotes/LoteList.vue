...
const { itens, carregar } = useLista('/vacinas/lotes/', 'Não foi possível carregar lotes.')

const nomeVacina = useMapaNomes('/vacinas/', (v) => v.nome)
const nomeUnidade = useMapaNomes('/unidades/', (u) => u.nome)

function novo() {
  router.push({ name: 'lote-form' })
}
function editar(id) {
  router.push({ name: 'lote-form', params: { id } })
}
...
<template>
  <BaseLayout titulo="Lotes" voltar-para="home">
    <BaseButton @click="novo">+ Novo Lote</BaseButton>

    <p v-if="itens.length === 0" class="vazio">Nenhum lote cadastrado.</p>
    ...
    <ul class="lista">
      <li v-for="item in itens" :key="item.id" class="card">
        <div>
          ...
          <p>Validade: {{ item.data_validade }}</p>
          <p>Quantidade disponível: {{ item.quantidade_disponivel }}</p>
        </div>
        <div class="acoes">
